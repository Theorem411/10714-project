...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        if node.is_leaf():   
            ...
            grad = node.op.gradient_as_tuple(v_i, node) 

            for i, inp in enumerate(node.inputs):

                if inp not in node_to_output_grads_list:
                    node_to_output_grads_list[inp] = [grad[i]]
                
                else:
                    node_to_output_grads_list[inp].append(grad[i])
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    for n in node_list:
        visited = set()
        ordering = []
        topo_sort_dfs(n, visited, ordering)
        res.append(ordering)
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for chldrn in node.inputs:
    
        topo_sort_dfs(chldrn, visited, topo_order)
