...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self.shape) != prod(new_shape) or not self.is_compact():
            raise ValueError("Shapes don't match in reshape with\n old shape: {self.shape} \tnew shape: {new_shape}")

        new_strides = [1 for _ in range(len(new_shape))]
    ...
    def broadcast_to(self, new_shape):
        ...
        if any([self.shape[i] != new_shape[i] and self.shape[i] != 1 for i in range(len(new_shape))]):
            raise AssertionError("Shapes don't match in broadcast_to with\n old shape: {self.shape} \tnew shape: {new_shape}")
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_strides = [s for s in self.strides]
        new_offset = self._offset
        ...
        res = NDArray.make(self.shape, strides=tuple(new_strides), device=self.device, handle=self._handle, offset=new_offset).compact()
        return res
    ...
    def pad(self, axes):
        ...
        res = NDArray.make(shape=new_shape, strides=self.strides, device=self.device, offset=0).compact()
        res.fill(0.0)
        
        slices = []
        for i in range(len(axes)):
            l_pad = axes[i][0]
            slices.append(slice(l_pad, l_pad + self.shape[i]))
...
def broadcast_to(array, new_shape):
    return array.compact().broadcast_to(new_shape)
...
def sum(a, axis=None, keepdims=False):
    return a.compact().sum(axis=axis, keepdims=keepdims)

def flip(a, axes):
    return a.compact().flip(axes)
